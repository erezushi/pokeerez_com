...
const detailsParser = (detailsObject: EvolutionDetail) => {
  ...
  switch (detailsObject.trigger.name) {
    ...
    case 'use-item':
      text = `using a${detailsObject.item?.name.match(vowels) ? 'n' : ''} ${startCase(
  ...
  Object.entries(detailsObject)
    .filter(
      ([key, value]) => !(key === 'trigger' || key === 'item') && (Boolean(value) || value === 0),
    )
    .forEach(([key, value]) => {
      switch (key) {
        case 'gender':
        ...
        case 'known_move':
          text = `${text} ${text.endsWith('Style') ? '' : 'while knowing '}${startCase(
            value.name,
          )}`;
          break;
        ...
        case 'party_type':
          text = `${text} while having a ${startCase(value.name)} type Pokémon in your party`;
          break;
...
const locations: Record<string, string[]> = {
  'a strong magnetic field': ['mt-coronet', 'chargestone-cave', 'kalos-route-13'],
...
export const chainFormatter = (chainObject: Chain): string => {
  ...
  if (chainObject.evolution_details.length) {
    const locationEvolutions = chainObject.evolution_details.filter((detailsObject) =>
    ...
    if (locationEvolutions.length) {
      text = `${text} by levelling up next to ${Object.keys(locations).find((key) =>
        locations[key].includes(locationEvolutions[0].location!.name),
      )}`;
    }

    if (nonLocationEvolutions.length) {
      text = `${text}${locationEvolutions.length ? ', or' : ''} by ${nonLocationEvolutions
